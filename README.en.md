...
These four works reuse the same four independent sources as the 16:9 set above, now regenerated as complete 3:4 top–bottom canvases. The reality photograph stays above; the lower design follows Panel 035's original brief. English copy is generated only from each photograph's content, mood, or implication, and every result was individually reviewed.
...
**Panel 035** is suited to work that must retain the source's essential content while pursuing this visual direction: **Rebuild the photograph as one vivid premium voxel-block collectible**. It begins with intended use and desired outcome; layout, size, text, and delivery format follow from those needs.
...
> Use XXD Panel 035 on this image and recommend the best layout and size first.
...
/xxd-panel-035 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB
...
/xxd-panel-035 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-035 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 035 to a whole directory; shared settings resolve once
/xxd-panel-035 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB
