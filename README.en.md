...
The repository reserves [`assets/examples/`](assets/examples/) for future work. Only finished 035 artwork verified by the project owner will be added; no post or image from another style is used as a placeholder.

Future samples will demonstrate 035's adaptability. Their subjects, whitespace ratios, palette, copy, and canvas ratios will never become generation references or defaults.

## Four outputs, one premium block-art system
...
| --- | --- | --- |
| `top-bottom` | source-adaptive | complete source above, 035 vivid voxel-block collectible below; both panels retain the source size and split exactly 50/50 |
| `left-right` | source-adaptive | complete source left, 035 vivid voxel-block collectible right; both panels retain the source size and split exactly 50/50 |
...
Exact user pixels > explicit ratio or destination > source adaptation for ordinary modes. The original `035.md` used a 3:4 creative canvas, but that historical example is not a silent default in the current Skill.
...
- **Independent set:** every device references only the source photograph and may explore different block scale, module density, local exaggeration, base, colour grouping, whitespace, and typographic alignment.
...
The image model creates the block silhouette, modular hierarchy, vivid source colour, matte ABS, visible seams, stepped thickness, natural contact shadows, quiet field, and native modular typography. `scripts/compose_panel.py` only plans canvases, performs exact 50/50 raster composition, finalises dimensions, and audits results. It never fakes artwork with programmatic drawing.
