...
| 模式 | 尺寸逻辑 | 成品 |
| --- | --- | --- |
| `top-bottom` | 源图自适应 | 上方完整原图，下方 035 明艳体素积木收藏品；两块都保持原图完整尺寸，严格 50/50 |
| `left-right` | 源图自适应 | 左侧完整原图，右侧 035 明艳体素积木收藏品；两块都保持原图完整尺寸，严格 50/50 |
| `design-only` | 源图自适应 | 只显示变化设计，不显示原照片；沿用原图比例和尺寸 |
| `wallpaper-pack` | 四种设备尺寸 | 分别输出手机、iPad、电脑、儿童手表四张 PNG |

用户精确尺寸 > 指定比例或用途 > 普通模式源图自适应。原始 `035.md` 里的 3:4 只是一开始的创作画幅，不会被写成当前 Skill 的静默默认值。
...
- **四张独立：** 每张只参考原照片，可以分别探索积木尺度、模块密度、局部夸张、底座、色组、留白与文字对齐。
...
图像模型负责积木剪影、模块层级、明艳源图色、哑光 ABS、清晰拼缝、阶梯厚度、自然接触阴影、安静背景和目标语言模块文字。`scripts/compose_panel.py` 只负责画布规划、精确 50/50 位图拼合、最终尺寸和审计，不会用程序绘图伪造成品。
