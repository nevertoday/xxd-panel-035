...
Use the host's built-in bitmap image-generation capability by default and follow the available `imagegen` skill when exposed. View each local source before generation. Use one generation call per distinct asset; a wallpaper pack requires four calls. “Voxel”, “building block”, “module”, “ABS plastic”, “pixel”, and “instruction-manual typography” describe raster image appearance only and never authorise SVG, HTML, CSS, Canvas, diagrams, or programmatic drawing as the artwork.
...
1. Resolve one or more modes. If missing, ask exactly this as normal multiline chat text and wait:
   ...
   1. 上下双联（原图在上＋035 设计在下）
   2. 左右双联（原图在左＋035 设计在右）
   3. 纯设计版（只显示 035 设计画面）
   4. 四端壁纸套装
      手机＋iPad＋电脑＋儿童手表
