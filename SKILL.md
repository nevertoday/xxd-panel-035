---
name: xxd-panel-035
description: "Create XXD Panel 035 artwork from supplied photos in four combinable modes: photo above/bright voxel-block sculpture below, photo left/design right, design alone, or a four-device wallpaper pack with independent or anchor-linked continuity. Rebuilds the source as one recognisable LEGO/Minecraft-like modular object with vivid source-derived colours, real matte ABS-plastic material, clear joints and contact shadows, a quiet low-information field, generous whitespace, and restrained pixel or instruction-manual typography. Use for the exact 035 premium block-art collectible style; never use it for complex Minecraft scenes, generic game screenshots, loose-brick floors, forced anthropomorphism, children's UI, or e-commerce toy displays."
---

# XXD Panel 035 · 明艳像素积木收藏品重构

Turn each supplied photograph into one vivid premium block-art collectible. Preserve the real photograph only in paired modes. First understand the source's identity, action, relation, and implication, then rebuild the defining subject as one recognisable LEGO/Minecraft-like voxel sculpture through block proportion, modular geometry, assembly logic, clear silhouette, layered thickness, vivid source-derived colour, matte ABS plastic, visible joints, and believable contact shadows. The environment retreats into a quiet low-information field with only a necessary base, plane, or shadow.
...
- The transformed frame must preserve identity, action, function, emotion, and relation through at least three source-specific cues while rebuilding the subject as one modular voxel/block sculpture. It is not a full Minecraft environment, loose-brick floor, forced mascot, generic game asset, low-poly mesh, children's interface, toy-shop render, smooth monolith, or e-commerce product shot.
...
Modes and device ratios may change placement and aspect ratio but never the 035 motive. Every transformed frame must visibly express this chain:

**this exact photographed subject or inseparable relation → preserve at least three defining cues across contour, pose, direction, proportion, opening, overlap, functional part, colour, and relational distance → convert the subject into a hierarchy of cubic and modular units → establish one clean silhouette with source-earned local exaggeration → assign vivid, high-purity source-derived block colours → expose believable seams, thickness, levels, and contact shadows in matte ABS plastic → keep one subject as the absolute focal point on a much lighter quiet field → align one concise native title and only useful pixel or instruction-manual microcopy to the silhouette, base, or whitespace**.

The collectible quality comes from treating voxel and building-block logic as physical design construction rather than a video-game filter. LEGO, Minecraft-like voxel grammar, matte injection-moulded ABS, collectible toy sculpture, instruction diagrams, and modern editorial display are disciplines rather than licences to copy a branded set or game scene.

Reject the result as generic when an unrelated photo could replace the source without materially changing the block silhouette, modular hierarchy, local exaggeration, colour identity, seam placement, base relation, or annotation. Also reject complex Minecraft terrain, castles or forests added without evidence, scattered bricks, game HUD, pixel sprites, low-poly faceting, glossy CGI, metal or glass, neon, pastel toy colour, forced faces, and commercial packaging.

## 035 visual system

### One recognisable modular subject
...
- Delete irrelevant detail and retain the structure, direction, silhouette, functional parts, opening, overlap, distance, and relation that make the source memorable.
...
- Rebuild with voxels, bricks, modular geometry, and plausible assembly logic rather than drawing square pixels over a cartoon. Recognition must survive thumbnail scale.
- Do not anthropomorphise animals, plants, buildings, vehicles, objects, or landscapes unless the source itself supports that reading.

### One focal collectible and a quiet field

- Keep one block-built subject as the absolute visual core with a clean silhouette and clear modular hierarchy.
- Use only a minimal base, plane, or soft contact shadow needed to support it. Do not reconstruct the full environment or add unrelated props, terrain, or scattered pieces.
- Let source direction and balance decide centred or off-centre placement, while a much lighter, simpler background supplies generous whitespace.
- Align annotation to the subject silhouette, base, block seam, or whitespace axis. The connection should feel designed, not pasted on.

### Matte ABS and credible assembly

- Use real matte ABS-like plastic with smooth flat faces, restrained soft reflections, visible module seams, credible thickness, stepped layers, and natural contact shadows.
- Every module must appear connectable or supportable. Keep joints, offsets, overlaps, and weight distribution plausible without turning the image into a literal construction manual.
- Use soft clear light that keeps colour vivid and reveals thickness and structure without dramatic studio gloss.
- Reject metallic or glass material, mirror highlights, excessive subsurface glow, over-polished CGI, mushy seamless blobs, cheap game rendering, and low-poly triangular facets.

### Vivid source-derived block colours

- Extract the source's most recognisable and spirited colours, then clarify them into high-purity, high-saturation block colours with clean hue boundaries.
- Preserve the source's combined colour identity while allowing the subject to become brighter and more energetic. Use area, value, and module grouping to maintain hierarchy.
- Keep the background clearly lighter and simpler so the saturated block subject remains the absolute focus.
- Avoid fixed primary-colour kits, neon, fluorescent hues, candy gradients, Morandi palettes, pastelisation, grey haze, muddy colour, and saturation reduced merely to appear tasteful.

## Copy behaves like modular auxiliary design

035 text is a restrained auxiliary layer derived from subject identity, action, emotion, relation, or supported meaning—not a toy logo, game UI, retail label, detached caption block, or decorative pixel font.

- Automatic copy derives one concise title from subject identity, action, emotion, relation, or supported meaning without inventing a brand, set number, game level, or product line.
- Prefer one restrained title plus zero to two grounded tag words or micro-notes. Numbers appear only when supplied or reliably established.
...
Typography has its own 035 construction:

- **Role:** a small information layer that supports the collectible without competing with it.
- **Voice:** concise, clear, lively, precise, and contemporary.
- **Construction:** one short title plus zero to two useful tags or micro-notes; the block subject remains the visual centre.
- **Placement:** align with the silhouette, base, block seam, module rhythm, or active whitespace.
- **Material:** restrained modular, pixel, or instruction-manual typography; never a game HUD, toy logo, oversized arcade font, faux set number, or retail packaging.
...
Prefer GPT Image 2 through the host's built-in bitmap image-generation capability when exposed, and follow the available `imagegen` skill for the built-in execution details. View each local source before generation. Use one generation call per distinct asset; a wallpaper pack requires four calls. “Voxel”, “building block”, “module”, “ABS plastic”, “pixel”, and “instruction-manual typography” describe raster image appearance only and never authorise SVG, HTML, CSS, Canvas, diagrams, or programmatic drawing as the artwork.
...
Use only images attached to the current invocation, explicit user paths, or a previously supplied image that the current request clearly identifies as “the same image”. Never broadly scan the Desktop, workspace, `~/Desktop/xxd/xxd-panel-035/`, or historical task folders to find a substitute source. If the intended source cannot be accessed, ask for it.
...
1. Resolve one or more modes. In a native multi-select use these human-facing labels: `上下对照`, `左右对照`, `只要设计图`, and `四端壁纸`. If no real multi-select exists, ask and wait:
   ...
   1. 上下对照（原图在上，035 设计在下）
   2. 左右对照（原图在左，035 设计在右）
   3. 只要设计图（成品中不显示原图）
   4. 四端壁纸
      手机、iPad、电脑、儿童手表各一张
...
5. Start a fresh job and reserve collision-safe output directories before generation. Use only the current invocation's explicit source or theme. Read `references/xxd-panel-035-prompt.en.md` or `references/xxd-panel-035-prompt.zh-CN.md` immediately before building the generation request.
...
7. Use **single complete-canvas generation as the default for every mode**:
   - `top-bottom`: supply the source as a high-fidelity edit/reference input and generate one finished image containing the faithful source in the upper half and the 035 transformation in the lower half.
   - `left-right`: supply the source as a high-fidelity edit/reference input and generate one finished image containing the faithful source on the left and the 035 transformation on the right.
   - `design-only`: generate the 035 transformation across the whole canvas; the source is reference-only and not visible.
...
10. Inspect the actual bitmap at full size and thumbnail size. Check, in order: whole-poster integration; 035 aesthetic fidelity; source identity and structure; visual and semantic correspondence between regions; typography and locale; mode, ratio/pixels, count, and PNG format; then approximate 50/50 geometry.
...
COPY PAYLOAD: <the exact 035-specific title and supporting-text package resolved under the local production prompt>
COPY RULE: Render only the populated strings in COPY PAYLOAD, each exactly once. Do not rewrite, translate, spell-correct, duplicate, or add text. Use native shaping, direction, punctuation, spacing, and semantic line breaks. Preserve the 035-specific hierarchy, amount of supporting text, placement, material, and typographic role instead of applying a generic overlay.
```
...
`scripts/compose_panel.py` remains available for deterministic recovery and audit. Trigger it only under step 11. When used, generate a same-aspect design asset from the full 035 prompt, preserve the source without destructive crop or stretch, and document which fallback condition applied. A direct complete-canvas success must not be split and recomposed again.
...
Save finished work under `~/Desktop/xxd/xxd-panel-035/` unless the user supplies another destination. Create the shared `~/Desktop/xxd/` wrapper, the skill root, and each task directory when needed.
...
- The block-built subject remains recognisable through at least three source-specific cues and preserves action, function, direction, or inseparable relation without forced anthropomorphism.
- One subject is the absolute focal point; only a necessary base, plane, or contact shadow remains, and no complex game environment, unrelated prop, terrain, or loose-brick floor survives.
- Voxel and modular hierarchy, silhouette, seams, stepped layers, thickness, joints, overlap, and weight distribution form plausible assembly rather than a square-pixel filter.
- High-purity, high-saturation colours come from this source and remain clean under a much lighter background; no fixed primary kit, neon, candy gradient, pastel, Morandi, grey haze, or muddy colour survives.
- Matte ABS faces, restrained plastic highlights, clear joints, and natural contact shadows feel materially credible without metal, glass, mirror gloss, excessive CGI, or low-poly facets.
- Automatic copy is concise, source-bound, and limited to one title plus zero to two useful tags or micro-notes. Exact user copy remains verbatim. All rendered text is accurate and native to the resolved locale; text-free output contains no text or pseudo-text.
- Modular, pixel, or instruction-manual type aligns with silhouette, base, seam, module rhythm, or whitespace and never becomes game HUD, toy branding, faux set metadata, or retail packaging.
...
User instructions may change subject emphasis, block scale, module density, local exaggeration, colour intensity, base treatment, whitespace, type relation, or copy within 035, but do not silently relax one-photo isolation, exact paired geometry, four separate wallpaper outputs, fresh-task generation, source-hidden output rules, or native-language typography. Leave the vivid premium voxel-block collectible aesthetic only when the user explicitly asks to leave the 035 style.

## Provenance boundary

The user's original style brief is archived at [references/035-source.md](references/035-source.md). It records the initial 3:4 top-bottom example but does not override the operative mode and explicit final-canvas selection and complete-canvas generation rules. The full local 035 prompt is the production specification. Never borrow subjects, colours, copy, or compositions from samples or previous outputs.
