---
name: xxd-panel-035
description: "Create XXD Panel 035 raster artwork from one image or a directory batch while treating the bundled original style brief as the sole aesthetic authority. Supports one or more top-bottom, left-right, design-only, and four-device wallpaper outputs; multiple ratios or exact sizes; prompt-generated, user-exact, or text-free typography; and inline parameters. Use when the user invokes xxd-panel-035 or requests the exact Panel 035 style."
---

# XXD Panel 035

Create fresh PNG artwork with `references/035-source.md` as the **sole creative and aesthetic authority**. This Skill is a runtime shell: it resolves delivery variables, appends them to the untouched source-brief body, executes bitmap generation, and verifies the result. It must not write a second art direction.

## Authority boundary

1. Read `references/035-source.md` completely immediately before building every generation request.
...
```text
/xxd-panel-035 <source-or-directory> --mode top-bottom,design-only \
...
```text
[VERBATIM SOURCE-BRIEF BODY FROM references/035-source.md,
...
```text
~/Desktop/xxd/xxd-panel-035/<fresh-task>/
...
- the result follows `references/035-source.md`, especially its own colour, material, composition, whitespace, and typography requirements, without an outer Skill palette or added art direction;
...
- `references/xxd-panel-035-prompt.zh-CN.md`
- `references/xxd-panel-035-prompt.en.md`

These files document the same minimal adapter. They never replace `references/035-source.md`.
