# XXD Panel 035｜运行适配器（中文）

本文件不是第二套美学提示词。生成时必须先完整读取 `references/original-prompt/zh-CN.md`；它是 035 的唯一创作与审美权威。首行 Markdown 标题只是内部归档标签，不进入生图请求；除此之外的原始提示词正文逐字使用。本文件只负责把用户本次选择的运行变量追加到正文末尾。
...
```text
035-source.md 原始提示词正文（除内部归档首行外，完整、逐字）
+ 公共交付前言
+ 一个已选模式块
+ 一种文字模式块
